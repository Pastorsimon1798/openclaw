...
def cmd_resize(args):
    """Resize existing photos for web/social"""
    if not HAS_PIL:
    ...
    # Find all photos
    files = [f for f in os.listdir(piece_dir) if f.endswith('.jpg')]
    
    if not files:
        print(f"❌ No photos found in: {piece_dir}")
        return 1
    
    size = SIZES.get(args.size, SIZES['web'])
    output_dir = os.path.join(piece_dir, args.size)
    Path(output_dir).mkdir(exist_ok=True)
    
    print(f"🔄 Resizing {len(files)} photos for {args.size} ({size[0]}x{size[1]})")
    
    processed = 0
    for filename in files:
        input_path = os.path.join(piece_dir, filename)
        output_path = os.path.join(output_dir, filename)
        
        if process_image(input_path, output_path, size, quality=args.quality):
            processed += 1
...
def cmd_list(args):
    ...
    for ph in photos:
        ...
        print(f"   Added: {ph['timestamp']} | {ph['width']}x{ph['height']}")
...
Examples:
  ...
  ceramics-photo resize piece-123 --size social
  ...
  ceramics-photo info
    ...
    # Resize command
    resize_parser = subparsers.add_parser('resize', help='Resize existing photos')
    resize_parser.add_argument('piece_id', help='Piece ID')
    resize_parser.add_argument('--year', help='Year directory')
    resize_parser.add_argument('--size', choices=list(SIZES.keys()), required=True)
    resize_parser.add_argument('--quality', type=int, default=90,
